...
Query GDELT (from Leviathan Google Cloud Account) to generate raw dataset of "risk" events
```
{ 
  SELECT ActionGeo_Lat, ActionGeo_Long,
  sum(if(EventRootCode='13' 
  OR EventRootCode='14' 
  OR EventRootCode='15' 
  OR EventRootCode='16' 
  OR EventRootCode='17' 
  OR EventRootCode='18' 
  OR EventRootCode='19',1,0)) as polriskevents
  FROM [gdelt-bq:gdeltv2.events] 
  WHERE IsRootEvent=1
  AND (SQLDATE>20170301 AND SQLDATE<20170525)
  AND (Actor1CountryCode='IZ' OR Actor2CountryCode='IZ' OR ActionGeo_CountryCode='IZ')
  GROUP BY 1, 2, 3
  ORDER BY 1
}
```
